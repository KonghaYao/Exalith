...
server.tool(
    ...
    async (args) => {
        ...
        try {
            ...
            const formattedText = chunks.map((chunk: any, index: number) => {
    ...
    async (args) => {
        ...
        try {
            const response = await fetch(
                `${process.env.RAGFLOW_BASE_URL}/datasets?${queryParams}`,
                {
                    method: "GET",
                    headers,
                }
            ...
            const formattedText = data.data.map((dataset: any, index: number) => {
