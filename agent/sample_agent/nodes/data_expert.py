...
async def data_expert(state: SuperAgentState, config: RunnableConfig):
    ...
    async with MultiServerMCPClient(mcp_config) as mcp_client:
        ...
        expert = create_expert_agent(
            ...
            tools=tools,
            store=store,
            research_system_prompt="""你是一个专注于数据分析和信息收集的研究代理。你的职责是基于提供的工具检查数据，并生成一份关于数据情况、潜在问题和分析机会的全面报告。你需要先直接操作工具观察数据，不需要解释，然后生成报告。
...
2. 任务步骤
    ...
    - 报告编写结束后，请你判断是否需要进一步编写执行计划或者执行任务，继续找制定代理帮助
...
收到用户需求后，判断是否需要进一步观察数据
- 上文提供信息不足或者是需要严谨执行，就需要观察数据，请找 **research_agent** 帮忙

## 工作细节
0. 你的任务无需用户确认
1. 根据需要调用各种数据工具进行代码运行和数据分析
2. 默认输出一个新数据文件
3. 默认保留所有原始数据列
